...
Schema:
{
  ...
  try {
    ...
    console.warn('[LLM Brief] Raw response excerpt:', String(result.text || '').slice(0, 500));
...
function parseBriefResponse(text) {
  ...
  try {
    ...
    if (!normalized.executiveSummary) return null;
    return normalized;
  } catch {
    return null;
  }
...
function normalizeBrief(brief) {
  const executiveSummary = brief.executiveSummary
    || brief.executive_summary
    || brief.summary
    || brief.brief
    || '';
  const keyRisks = brief.keyRisks
    || brief.key_risks
    || brief.risks
    || [];
  const marketImplications = brief.marketImplications
  ...
  return {
    ...
    keyRisks: Array.isArray(keyRisks) ? keyRisks : [],
    marketImplications: Array.isArray(marketImplications) ? marketImplications : [],
