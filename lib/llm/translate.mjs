...
function normalizeTranslations(parsed) {
  ...
  return {
    zh: {
      ...
      taiwanForeignNews: Array.isArray(zh.taiwanForeignNews) ? zh.taiwanForeignNews : [],
    ...
    en: {
      ...
      taiwanForeignNews: Array.isArray(en.taiwanForeignNews) ? en.taiwanForeignNews : [],
...
function buildTranslationPayload(data) {
  return {
    ...
    taiwanForeignNews: (data.taiwanForeignNews?.items || data.taiwanForeignNews || []).slice(0, 30).map(item => ({
      headline: trimText(item.headline, 220),
      source: item.source || '',
...
function fillTextListFallbacks(translatedItems, originals, field = null) {
  const values = Array.isArray(translatedItems) ? [...translatedItems] : [];
  for (let i = 0; i < originals.length; i++) {
    if (field) {
      if (!values[i]?.[field]) values[i] = { ...(values[i] || {}), [field]: originals[i] || '' };
    } else if (!values[i]) {
      values[i] = originals[i] || '';
    }
  }
  return values.slice(0, originals.length);
}

function fillObjectListFallbacks(translatedItems, originals, fields) {
  const values = Array.isArray(translatedItems) ? [...translatedItems] : [];
  for (let i = 0; i < originals.length; i++) {
    const original = originals[i] || {};
    const current = { ...(values[i] || {}) };
    for (const field of fields) {
      if (!current[field]) current[field] = original[field] || '';
    }
    values[i] = current;
  }
  return values.slice(0, originals.length);
...
function applyPartialTranslationFallbacks(payload, normalized) {
  const output = normalizeTranslations(normalized);
  const languages = ['zh', 'en'];
  for (const lang of languages) {
    output[lang].tSignals = fillTextListFallbacks(output[lang].tSignals, payload.tSignals || []);
    output[lang].newsFeed = fillObjectListFallbacks(output[lang].newsFeed, payload.newsFeed || [], ['headline']);
    output[lang].taiwanForeignNews = fillObjectListFallbacks(output[lang].taiwanForeignNews, payload.taiwanForeignNews || [], ['headline']);
    output[lang].tgUrgent = fillObjectListFallbacks(output[lang].tgUrgent, payload.tgUrgent || [], ['text']);
    output[lang].ideas = fillObjectListFallbacks(output[lang].ideas, payload.ideas || [], ['title', 'rationale', 'risk']);
  }
  return output;
}
...
export async function generateDashboardTranslations(provider, data) {
  ...
  const taiwanForeignNews = payload.taiwanForeignNews?.length
    ? await translateTextList(provider, 'taiwanForeignNews', payload.taiwanForeignNews.map(item => item.headline), 'foreign Taiwan news headlines', 8)
    : null;
  ...
  normalized = applyPartialTranslationFallbacks(payload, {
    zh: {
      tSignals: tSignals?.zh || [],
      newsFeed: newsFeed?.zh || [],
      taiwanForeignNews: (taiwanForeignNews?.zh || []).map(headline => ({ headline })),
    ...
    en: {
      ...
      taiwanForeignNews: (taiwanForeignNews?.en || []).map(headline => ({ headline })),
